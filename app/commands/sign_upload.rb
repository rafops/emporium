...
class SignUpload
  ...
  def call
    if json_request['headers']
      ...
      publish :signed_headers, signature(string_to_sign, json_request)
    else
      encoded_policy = Base64.encode64(raw_post).gsub("\n", '')
      publish :signed_policy, signature(encoded_policy, json_request), encoded_policy
    end
  end
